...
describe("dynamoClient module", () => {
  ...
  describe("writeData", () => {
    it("writes an item to DynamoDB and returns item with generated id and timestamp", async () => {
      ...
      const result = await writeData({ title: "Test Song", artist: ["Artist A"] });
    ...
    it("throws error when docClient fails", async () => {
      ...
      await expect(writeData({ title: "Test" })).rejects.toThrow("Dynamo error");
    });
  });
  ...
  describe("getData", () => {
    it("returns item when found", async () => {
      ...
      vi.spyOn(docClient, "send").mockResolvedValue({ Item: mockItem } as never);
    ...
    it("returns null when item is not found", async () => {
      vi.spyOn(docClient, "send").mockResolvedValue({ Item: undefined } as never);
  ...
  describe("listData", () => {
    it("returns list of items from table scan", async () => {
      ...
      vi.spyOn(docClient, "send").mockResolvedValue({ Items: mockItems } as never);
